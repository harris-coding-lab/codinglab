...
shed_data <- read_dta("shed-data-abridged.dta")
```
...
We cannot ask people directly if they are "financially stable" rather one survey question asks how people would pay for a unexpected $400  expense (e.g. hospital bill). There are 9 options, captured in different columns as indicator variables.
...
```{r distribution, exercise = TRUE, exercise.setup = "data-with-fin-stability"}
```
...
```{r quiz-refused}
quiz(
  question("What is the modal response?",
    answer("7"),
    answer("6"),
...
1) Create a new indictor variable called `trusting` which is `TRUE` when
...
```{r trust, exercise = TRUE, exercise.setup = "data-with-fin-stability"}
