...
We've provided a clean dataset of student loan data for you for this assignment. We ran the following code on this [student loan Excel dataset](https://github.com/harris-coding-lab/codinglab/raw/master/inst/tutorials/reading-files/www/area_report_by_year.xlsx) (click to download to your computer) to get a clean version, which you can [download here](https://github.com/harris-coding-lab/codinglab/raw/master/inst/tutorials/reading-files/www/student_loan_debt.csv).

The original student loan data looks like this:

![](images/01_excel.jpg){#id .class width=50% height=50%}

Here's the code we wrote to clean our data:
...
```{r question-types}
question("What type did the `year` column use to be? What is the new type of the `year` column?",
...
We can use the `arrange()` function from `dplyr` to sort the student loan data. The syntax is `arrange(data, variable)`. Can you sort the other way?
...
quiz(caption = "",
  ...
  question("How much was the lowest per capita dept in 2003?",
    answer("450"),
  ...
  question("How much did the average borrower owe in that state-year?",
    answer("15090"),
    answer("14020"),
    answer("13780"),
...
To tackle this, we'll use a population dataset from the same spreadsheet. We clean it with the following code, which is similar to what we did before. Here's the [cleaned dataset](https://github.com/harris-coding-lab/codinglab/raw/master/inst/tutorials/reading-files/www/population.csv) (click to download the csv).
...
Let's join the population data to our debt data and then we can weight the data by population.
