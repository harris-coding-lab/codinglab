...
Try to write the formula for standard deviation in R code using the `sqrt()`, `sum()`, and `length()` functions, along with other operators (`^`, `/`, `-`). Assign it to a new variable called `this_sd`.
```{r sd, exercise=TRUE}
...
```{r question-sd}
question("What number do you get for `this_mean`, or the standard deviation of `random_numbers`?",
...
question("What values do you get for `x_2` and `s_2`?",
  answer("x_2 is 1.975, s_2 is 0.932"),
  answer("x_2 is 1.975, s_2 is 0.946"),
  answer("x_2 is 1.975, s_2 is 0.965", correct = TRUE),
  answer("x_2 is 1.577, s_2 is 0.965"),
  allow_retry = TRUE
)
```
